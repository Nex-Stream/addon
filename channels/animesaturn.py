...
def get_data(item):
    try:
        ...
        support.logger.error(f"Errore durante il recupero dei dati: {e}")
        return []
...
def search(item, texto):
    ...
    try:
        ...
        support.logger.error(f"Errore nella ricerca: {e}")
        return []
...
def newest(categoria):
    ...
    try:
        ...
        support.logger.error(f"Errore nel recupero degli ultimi: {e}")
        return []
    return itemlist
...
def filter(item):
    ...
    for value, title in matches:
        itemlist.append(item.clone(title=support.typo(title, 'bold'),
                                   url='{}{}&{}%5B0%5D={}'.format(host, item.args, item.parameter, value),
                                   action='peliculas', args='filter'))
...
def peliculas(item):
    ...
    if item.args == 'top':
        ...
        if item.args == 'updated':
            ...
            typeContentDict = {'Movie': 'movie', 'Episodio': 'episode'}
            action = 'findvideos'
            def itemlistHook(itemlist):
                if page:
            ...
            def itemlistHook(itemlist):
                ...
                if page and item.nextpage < int(page):
                    itemlist.append(item.clone(title=support.typo(support.config.get_localized_string(30992), 'color kod bold'),
                                               url='{}&page={}'.format(item.url, item.nextpage),
                                               infoLabels={}, thumbnail=support.thumb()))
                return itemlist
        else:
