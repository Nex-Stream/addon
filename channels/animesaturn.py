...
def mainlist(item):
    ...
    return item  # Restituisce l'oggetto item
...
def search(item, texto):
    ...
    try:
        ...
        return item  # Restituisce l'oggetto item in caso di errore
...
def filter(item):
    ...
    for value, title in matches:
        item = item.clone(title=support.typo(title,'bold'), url='{}{}&{}%5B0%5D={}'.format(host, item.args, item.parameter, value), action='peliculas', args='filter')
    support.thumb([item], genre=True)
    return item  # Restituisce l'oggetto item
...
def play(item):
    ...
    return [item]  # Restituisce l'oggetto item in formato lista
